...
async def endpoint_save(request:Request,db:Session=Depends(get_db)):
    ...
    a=ep.auth or EndpointAuth(endpoint_id=ep.id);a.auth_type=f.get('auth_type') or 'none';a.username=(f.get('auth_username') or '').strip();a.header_name=(f.get('auth_header_name') or '').strip();a.hmac_algorithm=f.get('hmac_algorithm') or 'sha256';a.hmac_payload_basis=f.get('hmac_payload_basis') or 'raw_body';new_secret=f.get('auth_secret') or ''
...
async def endpoint_duplicate(eid:int,request:Request,db:Session=Depends(get_db)):
    ...
    ep.auth=EndpointAuth(auth_type=sa.auth_type if sa else 'none',username=sa.username if sa else '',secret_encrypted=sa.secret_encrypted if sa and f.get('include_secrets')=='on' else '',header_name=sa.header_name if sa else '',hmac_algorithm=sa.hmac_algorithm if sa else 'sha256',hmac_payload_basis=sa.hmac_payload_basis if sa else 'raw_body')
...
async def export_config_secrets(request:Request,db:Session=Depends(get_db)):
    ...
    for e in eps:data['endpoints'].append({'name':e.name,'description':e.description,'slug':e.slug,'active':e.active,'methods':e.methods,'content_types':e.content_types,'max_payload_bytes':e.max_payload_bytes,'rate_limit_per_minute':e.rate_limit_per_minute,'ip_allowlist':e.ip_allowlist,'ip_denylist':e.ip_denylist,'auth_type':e.auth.auth_type if e.auth else 'none','auth_username':e.auth.username if e.auth else '','auth_header_name':e.auth.header_name if e.auth else '','auth_secret':decrypt(e.auth.secret_encrypted) if e.auth and e.auth.secret_encrypted else ''})
